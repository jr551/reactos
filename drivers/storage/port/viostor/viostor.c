...
/*
 * This reduced miniport serializes I/O through Adapter->ActiveRequest, so it
 * owns exactly one request context. SrbExtension belongs to the caller until
 * storport grows real per-SRB extension allocation; classpnp stores its
 * original IRP there and VioSubmitIo must never overwrite it.
 */
                              ...
                              PVIOSTOR_SRB_EXTENSION Request)
{
    ...
    if (Adapter->ActiveRequest == Request)
        Adapter->ActiveRequest = NULL;
}
...
{
    DPrintf(0, "viostor: complete function=0x%02x opcode=0x%02x status=0x%02x scsi=0x%02x transfer=%lu\n",
                               ...
                               PSCSI_REQUEST_BLOCK Srb)
{
    ...
    OpCode = Srb->Cdb[0];
    DPrintf(0, "viostor: local opcode=0x%02x cdb_length=%u data_length=%lu\n",
                           ...
                           PSCSI_REQUEST_BLOCK Srb)
{
    ...
    DPrintf(0, "viostor: submit opcode=0x%02x flags=0x%08lx data_in=%u data_out=%u type=%u sector=%I64u bytes=%lu\n",
    ...
    DPrintf(0, "viostor: queued opcode=0x%02x type=%u sector=%I64u bytes=%lu sg=%lu out=%lu in=%lu request=%p header=%I64x status=%I64x\n",
...
static VOID VioCompletionDpc(PSTOR_DPC Dpc,
                             ...
                             PVOID SystemArgument2)
{
    ...
    if (!Adapter->CompletionPending) {
        KeReleaseSpinLock(&Adapter->Lock, Irql);
        return;
    }
    Request = Adapter->CompletionRequest;
    Srb = Adapter->CompletionSrb;
    Status = Adapter->CompletionStatus;
    DPrintf(0, "viostor: completion-dpc opcode=0x%02x used=%lu virtio_status=0x%02x queued=%p\n",
            Srb->Cdb[0], Adapter->CompletionLength, Request->Status,
            Adapter->QueuedSrb);
    Adapter->CompletionRequest = NULL;
    Adapter->CompletionSrb = NULL;
    Adapter->CompletionPending = FALSE;
    if (Adapter->ActiveRequest == Request) {
        Adapter->ActiveRequest = NULL;
        Adapter->CurrentSrb = NULL;
    }
    VioReleaseRequest(Adapter, Request);
    QueuedSrb = Adapter->QueuedSrb;
    Adapter->QueuedSrb = NULL;
    Queued = FALSE;
    if (QueuedSrb != NULL)
        Queued = VioSubmitIo(Adapter, QueuedSrb);
    KeReleaseSpinLock(&Adapter->Lock, Irql);

    VioComplete(Adapter, Srb, Status);
    if (QueuedSrb != NULL && !Queued)
        VioComplete(Adapter, QueuedSrb, SRB_STATUS_ERROR);

    /*
     * The completion DPC owns the one-request gate. Notify Storport only
     * after releasing it so queued class-driver requests can be admitted.
     */
    StorPortNotification(NextRequest, Adapter, NULL);
}
                                  ...
                                  PSCSI_REQUEST_BLOCK Srb)
{
    PVIOSTOR_ADAPTER_EXTENSION Adapter = DeviceExtension;
    KIRQL Irql;
    DPrintf(0, "viostor: startio function=0x%02x path=%u target=%u lun=%u opcode=0x%02x cdb1=0x%02x cdb2=0x%02x cdb4=0x%02x length=%lu\n",
    ...
    if (Adapter->CurrentSrb != NULL || Adapter->ActiveRequest != NULL ||
        Adapter->CompletionPending) {
        if (Adapter->QueuedSrb == NULL) {
            Adapter->QueuedSrb = Srb;
            Srb->SrbStatus = SRB_STATUS_PENDING;
            KeReleaseSpinLock(&Adapter->Lock, Irql);
            return TRUE;
        }
        KeReleaseSpinLock(&Adapter->Lock, Irql);
        VioComplete(Adapter, Srb, SRB_STATUS_BUSY);
        return TRUE;
    }
...
static BOOLEAN NTAPI VioHwInterrupt(PVOID DeviceExtension)
{
    ...
    /*
     * Storport already serializes HwInterrupt against the miniport's StartIo
     * path. Acquiring Adapter->Lock at DIRQL can deadlock a CPU that holds it
     * at DISPATCH_LEVEL, because the lock holder cannot run to release it.
     */
    QueueDpc = FALSE;
    if (!Adapter->Initialized || Adapter->Queue == NULL) {
        return FALSE;
    ...
    /*
     * Drain ALL pending used-ring entries. Returning TRUE without consuming
     * anything causes an interrupt storm on shared level-triggered INTx lines:
     * the kernel re-enters the ISR immediately, nothing deasserts the host,
     * and one core pins at 100%.
     */
    while ((Opaque = virtqueue_get_buf(Adapter->Queue, &Length)) != NULL)
    {
        Request = (PVIOSTOR_SRB_EXTENSION)Opaque;
        Srb = Request->Srb;
        if (Adapter->ActiveRequest == Request && Srb != NULL)
        {
            DPrintf(0, "viostor: used opcode=0x%02x length=%lu virtio_status=0x%02x request=%p\n",
                    Srb->Cdb[0], Length, Request->Status, Request);
            Adapter->CompletionLength = Length;
            Adapter->CompletionRequest = Request;
            Adapter->CompletionSrb = Srb;
            Adapter->CompletionStatus =
                Request->Status == VIRTIO_BLK_S_OK ?
                SRB_STATUS_SUCCESS : SRB_STATUS_ERROR;
            Adapter->CompletionPending = TRUE;
            QueueDpc = TRUE;
        }
        else
        {
            DPrintf(0, "viostor: used unexpected request=%p active=%p srb=%p\n",
                    Request, Adapter->ActiveRequest, Srb);
        }
    }

    if (QueueDpc)
    {
        StorPortIssueDpc(Adapter, &Adapter->CompletionDpc, NULL, NULL);
    }

    /* Return TRUE only if we actually consumed a completion; otherwise
     * let the kernel know this interrupt was not for us (shared line). */
    return QueueDpc ? TRUE : FALSE;
}
                                    ...
                                    PBOOLEAN Again)
{
    ...
    *Again = FALSE;
    ...
    StorPortInitializeDpc(Adapter, &Adapter->CompletionDpc, VioCompletionDpc);
